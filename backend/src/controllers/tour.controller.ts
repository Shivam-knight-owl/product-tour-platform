...
// Define orderBy type
type OrderByInput = Prisma.TourOrderByWithRelationInput & {
  views?: 'asc' | 'desc';
};

// Helper function to check tour ownership
const isOwner = async (tourId: string, userId: string): Promise<boolean> => {
...
export const getTours = async (req: Request, res: Response) => {
  try {
    ...
    const orderBy: OrderByInput = sort === 'views' 
      ? { views: 'desc' } 
      : { createdAt: 'desc' };
